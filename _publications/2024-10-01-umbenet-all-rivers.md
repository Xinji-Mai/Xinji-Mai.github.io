---
title: 'All Rivers Run into the Sea: Unified Modality Brain-Inspired Emotional Central Mechanism'
collection: publications
category: books
permalink: /publication/2024-umbenet-all-rivers/
date: 2024-10-01
venue: ACM International Conference on Multimedia (ACM MM)
paperurl: https://arxiv.org/abs/2407.15590
citation: 'Xinji Mai, Junxiong Lin, Haoran Wang, Zeng Tao, Yan Wang, Shaoqi Yan, et al. "All Rivers Run into the Sea: Unified Modality Brain-Inspired Emotional Central Mechanism." ACM MM 2024, 632-641.'
layout: publication
authors: Xinji Mai, Junxiong Lin, Haoran Wang, Zeng Tao, Yan Wang, Shaoqi Yan, et al
summary_en: Unifies multimodal emotion signals through a brain-inspired emotional central mechanism.
summary_zh: 通过脑启发的情感中枢机制，统一建模多模态情绪信号。
order: 7
selected: true
venue_short: ACM MM 2024
year: '2024'
codeurl: https://github.com/Xinji-Mai/UMBEnet
figure: /assets/images/research/umbenet.png
figure_alt: 'Figure 2: Overall architecture of UMBEnet'
figure_source: https://arxiv.org/html/2407.15590v1
excerpt: Unifies multimodal emotion signals through a brain-inspired emotional central mechanism.
redirect_from:
- /publication/2024-umbenet-all-rivers.html
figure_width: 1937
figure_height: 1111
---

<p class="i18n-en">Unifies multimodal emotion signals through a brain-inspired emotional central mechanism.</p>
<p class="i18n-zh">通过脑启发的情感中枢机制，统一建模多模态情绪信号。</p>
