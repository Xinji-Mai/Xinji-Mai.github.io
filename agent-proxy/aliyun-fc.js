...
http.createServer(function (req, res) {
  ...
  req.on("end", function () {
    ...
    var sys = "You are the strategic brain of an AI agent in a Terraria-like 2D mining sandbox. A low-level controller (pathfinding, digging, jumping, melee) executes the GOAL you choose; you only pick the next high-level goal plus a short first-person thought.\n" +
      "Grow stronger before taking risks: mine_ore upgrades pickaxe/sword, collect_gems upgrades armor, open_chest gives random upgrades and rarely LEGENDARY multiplier equipment or PERMANENT stat potions (both more likely in later worlds), hunt seeks out a beatable monster to farm. pillar_up climbs straight up by stacking dirt blocks (consumes dirtBlocks; use it to escape pits or head back toward the surface). Only fight/hunt enemies you can beat (enemy.beatable=true); avoid ones too strong (beatable=false) until geared. Seek the Grand Gem (seek_goal) only once it is known and you are well geared.\n" +
      "WORLD LAYOUT & PROGRESSION: gold/diamond get denser the DEEPER you dig; chests are denser toward the RIGHT; monsters are higher level deeper and further right; deep lava burns hard. Green MIASMA gas pockets drain hp steadily (gasNear warns you) BUT their walls hide the richest gold/diamond veins - a risk/reward play: only dash in with high hp (ideally a shield buff), mine fast, get out before the poison bleeds you. The VICTORY GATE sits at the FAR RIGHT on the surface, guarded by the gate BOSS; stepping through clears the world and starts a fresh one. Develop first (mine & upgrade near home); once power >= 5 push right and deeper for better loot; if upgrades stall in your area, stop grinding and explore right/deeper. Bosses grow +1 level each cleared world and your gear cap is TWICE the gate boss level; upgrades get pricier at higher levels. If the gate boss is NOT beatable yet, do NOT suicide into it - go back to developing (mine_ore, collect_gems, open_chest, hunt weaker monsters). Only when your power >= gateReq (the gate boss level; gateReq RISES by 1 each time you die challenging it - dying there means you were not ready) and hp is high, commit to pushing right, kill the gate boss and seek_goal. PERMANENT potions carry over across cleared worlds.\n" +
    ...
    var usr = "hp=" + s.hp + "/" + s.maxhp + ", power=" + s.power + ", gear pick/sword/armor=" + s.pick + "/" + s.sword + "/" + s.armor + ", gems=" + s.gems + ", dirtBlocks=" + s.dirt +
      ", depth=" + s.depth + ", exploredPct=" + s.exploredPct + ", goalKnown=" + s.goalKnown + ", deaths=" + (s.deaths || 0) + ", gateReq=" + (s.gateReq || 10) + ", event=" + (s.event || "none") + ", latent=" + (s.latent || "loot") +
      ", stuckSec=" + (s.stuckSec || 0) + ", bannedTargets=" + (s.bannedTargets || 0) + ", loiterSec=" + (s.loiterSec || 0) + ", lavaNear=" + !!s.lavaNear + ", gasNear=" + !!s.gasNear +
