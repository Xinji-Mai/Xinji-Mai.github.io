...
http.createServer(function (req, res) {
  ...
  req.on("end", function () {
    ...
    var usr = "hp=" + s.hp + "/" + s.maxhp + ", power=" + s.power + ", gear pick/sword/armor=" + s.pick + "/" + s.sword + "/" + s.armor + ", gems=" + s.gems + ", dirtBlocks=" + s.dirt +
      ...
      ", stuckSec=" + (s.stuckSec || 0) + ", bannedTargets=" + (s.bannedTargets || 0) + ", lavaNear=" + !!s.lavaNear +
    ...
    callLLM({ model: model, messages: [{ role: "system", content: sys }, { role: "user", content: usr }], max_tokens: 140, temperature: 0.7 })
      .then(function (d) {
        ...
        var H = ["explore", "explore_left", "explore_right", "mine_ore", "collect_gems", "dig_deep", "open_chest", "fight", "hunt", "avoid", "seek_goal", "surface", "pillar_up"];
        var plan = [];
